...
void printtimediff(char *name, num x) {
  ...
  time_t rt = time(NULL) - (running / 100);
  char buf[1024];

  strftime(buf, sizeof(buf), "%m.%d %H:%M", localtime(&rt));
  printf("%20s: %s (%u.%us)\n", name, buf, running / 100, running % 100);
}

wordint f77name(get_my_rss)(wordint *verbose) {
  tickspersec = sysconf(_SC_CLK_TCK);
  input = NULL;
  char filebuf[4096];

  snprintf(filebuf,4095,"/proc/%d/stat",getpid());
  input = fopen(filebuf,"r");
    if(!input) {
      perror("open");
      return -1;
    }
