...
static int Socket_Chunk_Size = 64*1024;         /* optimal packet size for transmissions */
...
/* set_chunk_size for packet transmissions */
void set_chunk_size(int size_K)
{
  
  Socket_Chunk_Size = size_K * 1024;
  fprintf(stderr,"DEBUG set_chunk_size to %d BYTES\n",Socket_Chunk_Size);
  
}

void f77name(set_chunk_size)(int *cs)
{
  set_chunk_size(*cs);
}
...
int write_stream(int fd, char *ptr, int n)  /*   %ENTRY%   */
{
  ...
  while (n > 0) 
    {
      if (select(fd+1, NULL, &wfds, NULL, &tv))
      	{
	  res = write(fd, ptr, (Socket_Chunk_Size < n ) ? Socket_Chunk_Size : n);
	  nw++;
#ifdef DEBUG
  ...
  fflush(stderr);
#endif
  fprintf(stderr, "DEBUG write_stream ntotal=%d chunk_size=%d, nw=%d\n",ntotal,Socket_Chunk_Size,nw);
  return n;
} 
...
int read_stream(int fd, char *ptr, int nbytes)  /*   %ENTRY%   */
{
  ...
  while (n > 0)
	...
	{
	  res = read(fd, ptr, n);
/*	  res = read(fd, ptr, (Socket_Chunk_Size < n ) ? Socket_Chunk_Size : n); */
          nr++;
#ifdef DEBUG
...
#endif   
  fprintf(stderr, "DEBUG read_stream ntotal=%d, nr=%d\n",nbytes,nr);
