...
static int FloatUnpacker_sse2(float *dest, void *src, int n, const int Shift2, const int Minimum_, const int MaxExp_, const int stream_32bit)
{
  ...
  scrap1 = _mm256_xor_si256(scrap1,scrap1);
  _mm256_zeroupper();
  if(stream_32bit){                            /* input stream contains 32 bit tokens */
    for (i0 = 0, j0 = 0 ; i0 < n0 ; i0 += VECTOR_STRIPE, j0 += SSE_VECTOR){
      ...
      _mm_storel_epi64(scrap,Mantis1);
      _mm256_zeroupper();
      scrap2 = _mm256_xor_si256(scrap1,scrap3);
      _mm256_zeroupper();
      _mm_storel_epi64(scrap,Mantis1);
...
#ifdef ORIGINAL
static INT_32 FloatUnpacker_new(float *dest, INT_32 *header, INT_32 *stream, int npts)
#else
static INT_32 FloatUnpacker_1(float *dest, INT_32 *header, INT_32 *stream, int npts)
#endif
{
  ...
  if (n != header[2]) {     /* verify that the number of points is consistent with header */
    printf("FloatUnpacker_1: ERROR inconsistent number of points, expected %d, got %d\n",n,header[2]);
...
#if defined(__SSE2__)
  n0 = FloatUnpacker_sse2(dest, stream, n, Shift2, Minimum, MaxExp, stream_32bit);
  if(n == n0) return(0);
  n = n - n0;
  stream += n0/2;
  dest += n0;
...
#ifdef TEST_PACK
static INT_32 FloatUnpacker_1_orig(float *dest, INT_32 *header, INT_32 *stream, int npts)
{
  floatint temp,temp2;
  INT_32 n;
  ...
  if (npts != header[2]) {     /* verify that the number of points is consistent with header */
    printf("FloatUnpacker_1: ERROR inconsistent number of points\n");
...
static INT_32 FloatUnpacker_1(float *dest, INT_32 *header, INT_32 *stream, int npts)
{
  return FloatUnpacker_1_orig(dest, header, stream, npts);
}
#endif
...
static INT_32 FloatPacker_1_new(float *source, INT_32 nbits, INT_32 *header, INT_32 *stream, int npts)
{
  float *z=source;
  ...
  n = (npts > 0) ? npts : -npts;
  if((*IsLittleEndian == 1) && (npts < 0)) stream_16bit=1;
...
static INT_32 FloatPacker_1(float *source, INT_32 nbits, INT_32 *header, void *stream, int npts)
{
  float *z=source;
  INT_32 *intsrc= (INT_32 *)source;
  floatint fmin,fmax,temp,temp2;
  ...
  n = (npts > 0) ? npts : -npts ;
  if((*IsLittleEndian == 1) && (npts < 0)) stream_16bit = 1;  /* storing result in shorts */
...
//fprintf(stderr,"shift2 = %d, round = %8.8x, Minimum = %8.8x, Minimum = %8.8x \n",Shift2,Round,Minimum,(Minimum>>Shift2)<<Shift2);
  ...
  Mantis = Mantis << Shift2;                         /* decode encoded maximum value */
  Mantis = Mantis + Minimum;                         /* regenerate mantissa, possibly not normalized */
  Sgn = (Mantis >> 31) & 1;
  if(Sgn) Mantis =- Mantis;                          /* need absolute value of Mantis */
  if (Mantis > 0xFFFFFF) Mantis = 0xFFFFFF;
  ...
  if(decoded > fmax.f) {              /* make sure decoded max will be <= actual max value     */
    if(MaxMax > 127) Mask=MaxMax-1;   /* BUT do not damage error on max value by more than 1%  */
  }
#endif
/* fprintf(stderr,"Debug+ MaxExp=%d\n",MaxExp);
  fprintf(stderr,"Debug+ min=%f fmin.i=%X max=%f Minimum=%d Maximum=%d\n",fmin.f,fmin.i,fmax.f,Minimum,Maximum); */
  n = (npts > 0) ? npts : -npts;
  if(stream_16bit){   /* normal order for tokens stored into shorts */
    high = 1;
    low = 0;
  }else{              /* short to int swap on little endian machines */
    high = 0;
    low = 1;
  }
  n3 = n & 0x7FFFFFFE;
  Src = intsrc[0];
  i = 0;
  Round = Round - Minimum;                  /* combine rounding and minimum removal */
  for( ; i<n3 ; i+=2){   /* process pairs of numbers */
    Src2 = intsrc[i+1];                     /* fetch next source values as early as possible */
    Src3 = intsrc[i+2];
    ...
    Mantis = (1 << 23) | ( 0x7FFFFF & Src2 );
    Exp    = (Src2 >> 23) & 0xFF;
    Shift  = MaxExp - Exp;
    if (Shift > 31) Shift = 31;
    Mantis = Mantis >> Shift;
    if( Src2 & 0x80000000 ) Mantis = - Mantis;
    Mantis = Mantis + Round;                /* subtract minimum from mantissa while rounding */
    Mantis = Mantis >> Shift2;              /* force to fit within nbits bits */
    if (Mantis > Mask) Mantis = Mask;
...
INT_32 FloatUnpacker(float *dest, INT_32 *header, void *stream, int npts_in, INT_32 *nbits)
{
  ...
  *nbits = ( (header[0]>>16) & 0xF) + 1 ;
  if(0xEFF != ( (header[0]>>20) & 0xFFF)) {
    printf("FloatUnpacker: ERROR invalid header \n");
    return -1;
    }
  if(npts != header[2]) {
    printf("FloatUnpacker: ERROR inconsistent number of points (header/request mismatch)\n");
    return -1;
    }
  ierror = FloatUnpacker_1(dest, header, stream, npts_in);
...
INT_32 FloatPacker(float *source, INT_32 nbits, INT_32 *header, INT_32 *stream, int npts)
{
  if(nbits > 16 || nbits < 1) {
...
/* =====================================================================================================
  ...
  ===================================================================================================== */
void FloatPacker_params(INT_32 *header_size, INT_32 *stream_size, INT_32 *p1, INT_32 *p2, int npts_in)
{
  int npts;
...
//  for ( i=0 ; i<NPTS ; i++ ) { source[i]=i*1.234-1123.123; }; source[10] = 0.0;
  ...
  FloatPacker(source, nbits, (INT_32 *)header, (INT_32 *)stream, npts);
...
//  for ( i=0 ; i< (1+2*((NPTS+32767)/32768))  ; i++ ) {
  ...
  gettimeofday(&t1,NULL);
  FloatPacker(source, nbits, (INT_32 *)header2, (INT_32 *)stream2, npts);
  gettimeofday(&t2,NULL);
  T1 = t1.tv_sec ; T1 = T1*1000000 + t1.tv_usec ;
  T2 = t2.tv_sec ; T2 = T2*1000000 + t2.tv_usec ;
  duree = T2-T1;
...
//  for ( i=0 ; i< (1+2*((NPTS+32767)/32768))  ; i++ ) {
  ...
  gettimeofday(&t1,NULL);
  FloatUnpacker_orig(source2, (INT_32 *)header2, (INT_32 *)stream2, npts, &NBITS);
  gettimeofday(&t2,NULL);
  T1 = t1.tv_sec ; T1 = T1*1000000 + t1.tv_usec ;
  T2 = t2.tv_sec ; T2 = T2*1000000 + t2.tv_usec ;
  duree = T2-T1;
  ...
  for ( i=0 ; i<NPTS ; i++ ) { source2[i]=-2000.; };
  gettimeofday(&t1,NULL);
  FloatUnpacker(source2, (INT_32 *)header, (INT_32 *)stream, npts, &NBITS);
  gettimeofday(&t2,NULL);
  T1 = t1.tv_sec ; T1 = T1*1000000 + t1.tv_usec ;
  T2 = t2.tv_sec ; T2 = T2*1000000 + t2.tv_usec ;
  duree = T2-T1;
...
//    }
  ...
  for ( j=0 ; j< 9 ; j++ ) {     /* perform repacking-unpacking cycles to verify stability */
    gettimeofday(&t1,NULL);
    FloatPacker(source2, nbits, (INT_32 *)header, (INT_32 *)stream, npts);
    gettimeofday(&t2,NULL);
    T1 = t1.tv_sec ; T1 = T1*1000000 + t1.tv_usec ;
    T2 = t2.tv_sec ; T2 = T2*1000000 + t2.tv_usec ;
    duree = T2-T1;
...
#ifdef FULL
    gettimeofday(&t1,NULL);
    FloatUnpacker_orig(source2, (INT_32 *)header, (INT_32 *)stream, npts, &NBITS);
    gettimeofday(&t2,NULL);
    T1 = t1.tv_sec ; T1 = T1*1000000 + t1.tv_usec ;
    T2 = t2.tv_sec ; T2 = T2*1000000 + t2.tv_usec ;
    duree = T2-T1;
