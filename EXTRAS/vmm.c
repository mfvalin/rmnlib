...
f77name(vmmwho)(wordint *adr)
{
   ...
   if ((adr < BLOCKS[0].memadr) || (adr > BLOCKS[nbblocks-1].memadr)) {
     fprintf(fdout,"VMMWHO address %p not in block table\n",adr);
     return(-1);
     }
   for (i=0; i < nbblocks; i++) {
	...
	fprintf(fdout,"\t          address = %p\n",adr);
	fprintf(fdout,"\t BLOCKS[%d].memadr = %p\n",i,BLOCKS[i].memadr);
	fprintf(fdout,"\t          indice  = %ld\n\n",1+(adr-BLOCKS[i].memadr));
 ...
 */
   ...
   fprintf(stdout,"Debug vmmend BLOCKS[0].memadr=%p\n",BLOCKS[0].memadr);
